...
if [[ -x ${SELF_DIRNAME}/computeset_job_notify.py ]]; then
    cd ${SELF_DIRNAME}
    ...
    ${SELF_DIRNAME}/computeset_job_notify.py >> $LOGFILE 2>&1
else
    echo "${SELF_DIRNAME}/computeset_job_notify.py not found/executable" >> $LOGFILE 2>&1
fi
